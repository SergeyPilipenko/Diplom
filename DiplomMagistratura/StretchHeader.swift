...
open class StretchHeader: UIView, UIScrollViewDelegate {
    
   // open var imageView : UIImageView!
    open var conteinerView : UIView!
    ...
    fileprivate func commonInit() {
       ...
       // addSubview(imageView)
        ...
        conteinerView.contentMode = .scaleAspectFill
        conteinerView.isUserInteractionEnabled = true
    ...
    open func stretchHeaderSize(headerSize: CGSize, imageSize: CGSize, controller: UIViewController, options: StretchHeaderOptions) {
        ...
        conteinerView.frame = CGRect(x: 0, y: 0, width: imageSize.width, height: imageSize.height)
    ...
    open func updateScrollViewOffset(_ scrollView: UIScrollView) {
        ...
        if conteinerView == nil { return }
        var scrollOffset : CGFloat = scrollView.contentOffset.y
        scrollOffset += topInset
        
        if scrollOffset < 0 {
           //imageView = CGRect(x: scrollOffset ,y: scrollOffset, width: contentSize.width - (scrollOffset * 2) , height: contentSize.height - scrollOffset)
            conteinerView.frame = CGRect(x: scrollOffset ,y: scrollOffset, width: contentSize.width - (scrollOffset * 2) , height: contentSize.height - scrollOffset);
        } else {
            //imageView.frame = CGRect(x: 0, y: 0, width: contentSize.width, height: contentSize.height)
            conteinerView.frame = CGRect(x: 0, y: 0, width: contentSize.width, height: contentSize.height);
        }
    }
}
